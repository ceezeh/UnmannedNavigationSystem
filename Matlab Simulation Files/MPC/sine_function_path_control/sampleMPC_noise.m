...
while (time < floor(Td/v)-100)
    ...
    normTheta = mod(theta,2*pi);
    normThetar = mod(thetar,2*pi);
    e0 = [xr-x;yr-y;normThetar-normTheta];%theta?
...
% Update robot's movement.
    ...
    lim = 0.1; %10 degrees
     n = -lim/2 + rand(1,1) *  lim;
%     n = normrnd(0, stnd);
    theta = theta + thetaDot + n;
    % theta wraparound
    if theta > pi
        theta = theta - 2*pi;
    elseif theta < -pi
    ...
    n = -noiselim/2 + rand(1,1) * noiselim;
%     n = normrnd(0, stnd);
    x = x + T*xdot + n; 
...
scatter(xSeq, ySeq);
scatter(xrSeq, yrSeq);
str = sprintf('Trajectory Comparision with error in x and y: unif(-%f, %f) and in theta: unif(-%f, %f)', noiselim/2, noiselim/2, lim,lim);
title(str);
